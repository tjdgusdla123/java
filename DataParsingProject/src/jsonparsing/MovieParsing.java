...
public class MovieParsing {
	...
	public static void main(String[] args) {
		...
		try {
			...
			// URL에 한글이 있으면 한글 부분은 utf-8로 인코딩해야합니다.
			// URLEncoder.encode("한글문;자열","utf-8")
			URL url = new URL(addr);

			// HttpURLCoonection생성
			// openConnection은 HttpURLConnection이라는 추상 클래스 타입으로 리턴하므로 강제 형 변환 해서 일반클래스 타입으로
			// 변경
			HttpURLConnection con = (HttpURLConnection) url.openConnection();
			// 옵션설정
			con.setConnectTimeout(3000);// 최대 연결 시간 설정
		...
		try {
			...
			// 이 메시지가 출력안되면
			// mysql Driver를 프로젝트에 삽입하지않았거나 드리이버 클래스 이름이 잘못됨

			// 데이터베이스 연결
			// mysql은 한글이 있는 경우 연결 문자열에 유니코드 사용을 명시해야 합니다.
			// ?useUnicode=true&characterEncoding=utf8

			// MySQL과 java의driver와 버전이 맞이 않아서
			// timeZone에러가 발생하느 경우 추가
			// &serverTimezone=UTC
