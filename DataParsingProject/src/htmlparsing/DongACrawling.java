...
public class DongACrawling {
	...
	public static void main(String[] args) {
		...
		try {
			...
			String addr = 
				"https://www.donga.com/news/search?p=1&query=" 
					+ query 
		...
		try {
			...
			System.out.println("기사 개수 가져오기 실패");
			System.out.println(e.getMessage());
			e.printStackTrace();
		}
		...
		try {
			//반복문 안에서 예외가 발생했을 때 다음 반복으로 
			//넘어가고자 하면 반복문안에서 예외 처리
			for(int i=0; i<pageCnt; i=i+1) {
				try {
					...
					query = URLEncoder.encode(query, "utf-8");
					String addr = 
						"https://www.donga.com/news/search?p=" 
							+ ((i*perPageCnt) + 1) 
							+ "&query=" + query 
							+ "&check_news=1&more=1&sorting=1"
							+ "&search_date=1&v1=&v2=&range=1";
